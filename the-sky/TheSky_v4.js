...
// ============================================================================
// SKY CONDITIONS — v4
// 6 time-of-day states. No clear/overcast split.
// Cloud density and coverage create the weather — not the gradient.
// HSB ranges are wide enough to encompass clear through hazy conditions.
// PLACEHOLDER VALUES — replace with photo-derived data.
// ============================================================================
...
// ============================================================================
// CONDITION SELECTION — v4
// Forward time sequence: night → dawn → sunrise → midday → sunset → dusk → night
// No clear/overcast branching. Each state is just one entry.
// From any condition, transitions to the next in the cycle.
// ============================================================================
...
function instantiateCondition(conditionKey) {
  ...
  // Handle hue ranges that wrap around 360 (e.g., sunset bottom: 340-85)
  function pickHue(range) {
    if (range[0] <= range[1]) {
      return R.random_num(range[0], range[1]);
    } else {
      // Wrapping range: e.g., [340, 85] means 340→360→0→85
...
// ============================================================================
// UNIFORM PACKING
// Convert a renderState's cloud array into shader uniforms.
