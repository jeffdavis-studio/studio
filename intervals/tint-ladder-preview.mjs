// intervals/tint-ladder-preview.mjs — rasterize the tint-ladder sheet so it can
// be LOOKED AT before it goes anywhere.
//
//   node intervals/tint-ladder-preview.mjs --out <dir> [--ppmm 4]
//
// Same method as white-loss-panels-preview.mjs: read the emitted pen files back,
// paint every segment at NIB width with round caps in that pen's own color, on
// white, in MULTIPLY. Not at the 1 px hint the files carry — that is a plotter
// instruction, not an ink width.
//
// The second image is the DIGITAL reference, and here it is six different
// renders, not one drawn six times: the artwork is re-rendered at each panel's
// forced tint (forceAnchorTint, svg-generator-v5.html) and captured into that
// panel's box. Held against the plotted sheet, the two ladders should step in
// the same places.
...
const meta = JSON.parse(readFileSync(join(OUT, 'tint-ladder.json'), 'utf8'));
...
writeFileSync(join(OUT, 'tint-ladder-preview.png'), Buffer.from(plotPng, 'base64'));

// ---- 2. the same six tints, digitally, at the same six boxes -------------------
const dig = await page.evaluate(a => {
  ...
  for (const p of a.panels) {
    forceAnchorTint(p.tint);           // re-renders the artwork at this tint
  ...
  return { png: c.toDataURL('image/png').split(',')[1], srcW: src.width, srcH: src.height, restored };
}, { w: DOC_W, h: DOC_H, ppmm: PPMM, panels: meta.layout.panels });
if (!dig.restored) throw new Error('the page was not left on the token as it mints');
writeFileSync(join(OUT, 'tint-ladder-digital-reference.png'), Buffer.from(dig.png, 'base64'));
...
console.log('digital reference: six renders from the artwork canvas ' + dig.srcW + ' x ' + dig.srcH +
            ', one per forced tint, page restored');
