// intervals/swatch-generator-v2-check.mjs — drives swatch-generator-v2.html in headless
...
// says they are; nothing outside the layout box; no zero-length or
// duplicated strokes.
//
// ADDED IN v2, 2026-09-10 — THE FRAME. The document is the plotter's working
// area, not the paper (Jeff: the old files plotted "cut off on the left and
// pushed left and down"). So this check also asserts, against the emitted
// bytes: root width 297mm x height 410mm, viewBox "0 0 297 410", no transform
// on the root, every drawn segment inside [6, 291] x [6, 404], and that no
// coordinate anywhere in any emitted file references the old paper/envelope
// frame (355.6, 431.8, 270, 370). The numbers come from plot-frame.mjs.
//
//   node intervals/swatch-generator-v2-check.mjs [outDir]
//
...
const OUT = path.resolve(process.argv[2] || path.join(process.cwd(), 'swatch-v2-out'));
...
async function main() {
  ...
  await page.goto(`http://127.0.0.1:${port}/swatch-generator-v2.html`, { waitUntil: 'load' });
  ...
  check(x0 >= bx0 - 1e-6 && x1 <= bx1 + 1e-6 && y0 >= by0 - 1e-6 && y1 <= by1 + 1e-6,
        `all ink inside [${bx0}, ${bx1}] x [${by0}, ${by1}] mm — the document less its ${DOC_MARGIN} mm frame`);
  const slack = Math.min(x0 - bx0, bx1 - x1, y0 - by0, by1 - y1);
  console.log(`        ink extent ${(x1 - x0).toFixed(1)} x ${(y1 - y0).toFixed(1)} mm ` +
              `at (${x0.toFixed(1)}, ${y0.toFixed(1)}) - (${x1.toFixed(1)}, ${y1.toFixed(1)})`);
  console.log(`        layout-box slack ${slack.toFixed(1)} mm at the tightest edge`);
  check(x0 > 0 && y0 > 0 && x1 < DOC_W && y1 < DOC_H, 'all ink inside the document');

  // The old paper frame must not survive anywhere in the emitted bytes: no
  // 355.6 / 431.8 paper, no 270 / 370 envelope, in any attribute or coordinate.
  ...
  await page2.goto(`http://127.0.0.1:${port}/swatch-generator-v2.html`, { waitUntil: 'load' });
