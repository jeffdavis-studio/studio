// intervals/mix-compare-generator-v2-check.mjs — drives mix-compare-generator.html
...
//
// 2. THE OTHER HALF IS RESERVED. In the original this was "Jeff has the 9/09
//    chart on this paper; its ink stops at x = 171.8" — a fact about a sheet
//    that is now finished. v2 carries the RULE, not the paper number: every
//    point of ink has to sit right of the DOCUMENT's mid-line (297/2 = 148.5)
//    plus the clear margin, asserted on the downloaded segments.
//
// 3. THE FRAME, added 2026-09-10. The document is the plotter's working area,
//    not the paper (Jeff: the old files plotted "cut off on the left and pushed
//    left and down"). Root width 297mm x height 410mm, viewBox "0 0 297 410",
//    no transform on the root, every drawn segment inside [6, 291] x [6, 404],
//    and no coordinate anywhere referencing the old paper/envelope frame
//    (355.6, 431.8, 270, 370). Numbers from plot-frame.mjs.
//
//   node intervals/mix-compare-generator-v2-check.mjs [outDir]
//
// Writes the SVGs and the proof PNGs to outDir (default ./compare-v2-out) and exits
...
const OUT = path.resolve(process.argv[2] || path.join(process.cwd(), 'compare-v2-out'));
...
async function main() {
  ...
  await page.goto(`http://127.0.0.1:${port}/mix-compare-generator-v2.html`, { waitUntil: 'load' });
  ...
  // The half-split is the rule now, against the DOCUMENT, not against a sheet.
  const MIDLINE = DOC_W / 2;
  check(near(geo.halfX0, MIDLINE, 1e-9),
        `the document's mid-line is ${MIDLINE} mm (${geo.halfX0}) — not the paper's 177.8`);
  ...
  check(x0 >= DOC_MARGIN - 1e-6 && y0 >= DOC_MARGIN - 1e-6 &&
        x1 <= DOC_W - DOC_MARGIN + 1e-6 && y1 <= DOC_H - DOC_MARGIN + 1e-6,
        `all ink inside [${DOC_MARGIN}, ${DOC_W - DOC_MARGIN}] x ` +
        `[${DOC_MARGIN}, ${DOC_H - DOC_MARGIN}] mm — the document less its frame`);
  ...
  for (const key of Object.keys(emitted)) {
    for (const f of OLD_FRAME) {
      if (f.re.test(frameOf(emitted[key].svg))) { oldHits++; oldWhere = `${key}: ${f.tok}`; }
    }
  }
  ...
  check(geo.docH - geo.cy1 >= 150,
        `${(geo.docH - geo.cy1).toFixed(0)} mm of the right half still free below the test`);
  ...
  await page2.goto(`http://127.0.0.1:${port}/mix-compare-generator-v2.html`, { waitUntil: 'load' });
  ...
  fs.writeFileSync(path.join(OUT, 'manifest.json'), JSON.stringify({
    ...
    footprint: { x0: +x0.toFixed(2), y0: +y0.toFixed(2), x1: +x1.toFixed(2), y1: +y1.toFixed(2),
                 contentBoxBottomY: geo.cy1, freeBelowMm: +(geo.docH - geo.cy1).toFixed(1) },
