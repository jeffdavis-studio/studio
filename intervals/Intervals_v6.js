// Intervals v6 — supersedes Intervals_v5.js. THE EMITTER IS IN THE ARTWORK NOW.
//
// v5 was the artwork and svg-generator-v5.html was a separate 2,087-line bench
// that loaded it, called setup() and coverage(), and did all the plot work
// itself. Nothing drifted — the bench never re-implemented the artwork — but
// the artwork could not export on its own, so every plot file had to come out
// of a page that was not the piece. v6 folds the engine in: ink weight -> hatch
// lines -> bar-edge clip -> serpentine order -> per-pen SVG all live here,
// under one PLOT block of named constants, behind one action,
// exportPlotFiles(). Nothing visible changes on the canvas. A viewer never sees
// any of it.
//
// WHAT IS NEW, AND ONLY THIS:
//   1. PLOT — every plot constant in one place, at the top of the file.
//   2. The engine, MOVED (not rewritten) out of svg-generator-v5.html. Where
//      the bench mixed UI state into the engine, the two are separated: the
//      engine reads PLOT, the UI (plot-bench-v6.html) writes PLOT.
//   3. exportPlotFiles() — key "p" on the page, or ?plot=1 in the URL.
//   4. ?hash=0x... pins the token, so an exported token is reopenable.
//   5. VARIANTS — a table of named parameter sets replacing v5's if/else
//      branch in setup(). ?variant=name forces a row.
//
// TINT IS THE ONE ARTWORK CHANGE. amax moves 0.60 -> 0.40, LOCKED by Jeff
// 2026-09-18 off the plotted tint ladder. That changes what every hash looks
// like, and that is expected: Jeff, 2026-09-19, "we do not need to try to
// maintain any sort of alignment with prior versions in terms of hash
// determinism. It's ok if v6 produces different looking artwork." v6 is
// verified against itself. Intervals_v5.js and svg-generator-v5.html are NOT
// touched and still render and emit exactly what they did.
//
// Everything else — ghue(), snap(), gcol(), mix(), coverage(), the geometry,
// the Lab lerp, the ink table — is v5's, moved or left alone.
...
const PLOT = {
  ...
  // --- GEOMETRY -------------------------------------------------------------
  // 11 in = 279.4 mm exactly (Jeff, 2026-09-11 — he thinks in inches and the
  // first plotted token is an 11 in square), centered in the document.
  imgW: 279.4,
  imgH: 279.4,
...
// ============ GEOMETRY ============
// THE FRAME IS NOT A CONTROL in the sense the image is. The document is the
// plotter's working area, 297 x 410 mm (plot-frame.mjs); the image is
// positioned inside THAT, not inside a sheet of paper. The default image is
// 11 in = 279.4 mm exactly, which leaves (297 - 279.4) / 2 = 8.8 mm of slack
// across and (410 - 279.4) / 2 = 65.3 mm down.
function plotGeometry() {
  const docW = PLOT.docW, docH = PLOT.docH, docMargin = PLOT.docMargin;
  const imgW = PLOT.imgW, imgH = PLOT.imgH;
  return {
...
// draw() lays 3s full-height bars edge to edge across the square, then rotates
// the whole field by r*90 about the center. A 90 degree rotation of a bar keeps
// it a rectangle — (x, y) maps to (S - y, x) — so the field stays axis-aligned
// and no general transform is needed.
...
// image boundary and no ink falls outside the stated square.
//
...
function clipRect(rect, bar, t, marks) {
  ...
  // rows at r = 90. The two edges across that direction are shared with a
...
function layerFilename(t, layer) {
  ...
  return 'intervals-v6-' + t.hash.slice(2, 10) + '-' + penIds()[layer.ink] + '-' + slug +
         '-' + layer.angle + 'deg.svg';
}
...
function penFilename(t, ink) {
  ...
  return 'intervals-v6-' + t.hash.slice(2, 10) + '-' + penIds()[ink] + '-' + slug + '.svg';
}
...
function barGroups(layer, pad) {
  const ids = penIds();
  return layer.bars.map((bar, i) => {
    const lineElements = bar.lines
      .map(l => pad + '  <line x1="' + l.x1.toFixed(6) + '" y1="' + l.y1.toFixed(6) +
    ...
    return pad + '<g id="' + i + '-bar-' + ids[layer.ink] + '-b' + bar.band + 's' + bar.step +
           ...
           '" data-clip="' + bar.clip.x.toFixed(6) + ' ' + bar.clip.y.toFixed(6) + ' ' +
...
// Inside a layer, as before. The lift BETWEEN two angle groups is the one
// figure the per-angle files never had to carry, so it is stated on its own
// rather than folded into data-pen-up-mm — which stays the sum of the layers.
