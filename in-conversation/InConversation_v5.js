...
//   Overlapping:   1 method  (volume)             [v5 new]
//   Diminution:    1 method  (volume)             [v5 new]
//   Perspective:   1 method  (volume)             [v5 new]
//   Volume:        1 method  (volume)             [v5 new]
//   Ambiguity:     0 methods [NONE]
//
// Summary: 36 subtopics. 13 unique wired (was 9 in v4), 23 unwired.
...
//
// v5 changes:
//   - volume: new method — projected 3D volumes (isometric + linear perspective)
//     Wired subtopics: Perspective, Volume, Overlapping, Diminution (Space)
//     Rough scaffolding: Square complete, Circle/Triangle/Line sketched
//
// v4 changes:
...
let methods = {
  ...
  // ---------------------------------------------------------------------------
  // VOLUME
  // Projected 3D volumes built from existing shape primitives.
  // Two projection modes: isometric (parallel, no foreshortening) and linear
  // perspective (converging to vanishing point(s)).
  //
  // Construction approach (from logo algorithm):
  //   1. Build a front face — a centrosymmetric quad (or shape-appropriate form)
  //   2. Compute a projection vector (dx, dy) — the "depth direction"
  //   3. Generate offset planes by translating the front face by +/-(dx, dy)
  //   4. Select which face-pairs to render (side faces connecting front to back)
  //   5. Layer faces with fill/stroke alternation and optional color stepping
  //
  // For isometric: projection vector has consistent magnitude (parallel edges
  // stay parallel). Standard angles: 30, 45, 60 degrees from horizontal.
  //
  // For linear perspective: projection vector converges toward a vanishing point.
  // Edges recede rather than staying parallel. 1-point (single VP on horizon)
  // or 2-point (two VPs, vertical edges stay vertical).
  //
  // Shape mapping:
  //   Square   -> cube / rectangular prism (parallelogram side faces)
  //   Circle   -> cylinder (ellipse caps + connecting tangent lines)
  //   Triangle -> triangular prism (triangle caps + parallelogram sides)
  //   Line     -> extruded line (the line + its projected copy + connecting edges)
  //
  // Knobs:
  //   projection:     "isometric" / "linear" / "random"
  //   depthAngle:     angle of the projection vector (degrees from horizontal)
  //   depthScale:     how deep the projection extends (fraction of canvas)
  //   faceCount:      how many face layers to render (1-4, like logo reps)
  //   colorScheme:    "single" / "stepped" / "binary" / "random"
  //                   "stepped" = logo modColor gradient across faces for depth
  //   outline:        probability of outlined vs filled faces
  //   count:          number of volumes (1 = single, 2+ = multiple/overlapping)
  //   vanishingPoint: for linear perspective — position on canvas
  // ---------------------------------------------------------------------------
  volume: {
    shapes: ["Line", "Circle", "Square", "Triangle"],
    defaults: {
      projection: "random",
      depthAngle: "random",          // degrees: 30, 45, 60 for iso; derived from VP for linear
      depthScale: "random",          // 0.1-0.4 of canvas
      faceCount: "random",           // 1-4 face layers
      colorScheme: "random",         // single, stepped, binary
      outline: 0.3,
      count: 1,                      // number of volumes
      vanishingPoint: "random"       // for linear: center, left, right
    },
    subtopics: {
      // Perspective: linear projection, clear VP convergence, single volume to
      // emphasize the spatial construction itself.
      "Perspective": { projection: "linear", count: 1 },
      // Volume: isometric with color stepping — the depth gradient makes the
      // volume feel solid and three-dimensional without perspective distortion.
      "Volume": { projection: "isometric", colorScheme: "stepped", outline: false },
      // Overlapping: multiple volumes that overlap — the spatial ambiguity of
      // stacked projected forms creates depth through occlusion.
      "Overlapping": { count: "random_multi", projection: "isometric" },
      // Diminution: linear perspective with multiple volumes receding toward VP.
      // Size decreases with distance, reinforcing depth through scale change.
      "Diminution": { projection: "linear", count: "random_multi" }
    },
    draw: function(shape, config) {
      let projection = resolveChoice(config.projection, ["isometric", "linear"]);
      let outline = chance(config.outline);
      let colorScheme = resolveChoice(config.colorScheme, ["single", "stepped", "binary"]);
      let faceCount = config.faceCount === "random" ? R.random_int(1, 4) : config.faceCount;
      let count = config.count === "random_multi" ? R.random_int(2, 4) : config.count;

      // --- Projection vector ---
      // Isometric: fixed angle, parallel edges. The angle determines which
      // "corner" of the volume faces the viewer.
      // Linear: derived from vanishing point position — edges converge.
      let depthAngle, dx, dy, depthMag;

      if (projection === "isometric") {
        // Standard isometric angles: 30 (classic), 45 (cabinet), 60 (steep)
        let isoAngles = [30, 45, 60];
        depthAngle = config.depthAngle === "random"
          ? R.random_choice(isoAngles)
          : config.depthAngle;
        depthMag = config.depthScale === "random"
          ? R.random_num(0.1, 0.35) * sd
          : config.depthScale * sd;

        // Projection direction — randomize left/right lean
        let sign = R.random_bool(0.5) ? 1 : -1;
        let rad = depthAngle * Math.PI / 180;
        dx = sign * Math.cos(rad) * depthMag;
        dy = -Math.sin(rad) * depthMag;  // up = negative y
      } else {
        // Linear perspective — vanishing point on canvas
        let vpChoice = resolveChoice(config.vanishingPoint, ["center", "left", "right"]);
        let vpx, vpy;
        if (vpChoice === "center") { vpx = sd / 2; vpy = sd * R.random_num(0.3, 0.5); }
        else if (vpChoice === "left") { vpx = sd * R.random_num(0.1, 0.3); vpy = sd * R.random_num(0.3, 0.5); }
        else { vpx = sd * R.random_num(0.7, 0.9); vpy = sd * R.random_num(0.3, 0.5); }
        // VP stored for edge convergence calculations in face rendering
        // (faces don't use a fixed dx/dy — each vertex converges toward VP)
        dx = vpx;  // repurpose as VP coordinates for linear mode
        dy = vpy;
        depthMag = config.depthScale === "random"
          ? R.random_num(0.15, 0.4) * sd
          : config.depthScale * sd;
      }

      // --- Build and render volumes ---
      for (let v = 0; v < count; v++) {
        // Per-volume placement — offset each volume's center for count > 1.
        // For linear perspective, later volumes are smaller (diminution).
        let cx = sd / 2 + (count > 1 ? R.random_num(-0.25, 0.25) * sd : 0);
        let cy = sd / 2 + (count > 1 ? R.random_num(-0.25, 0.25) * sd : 0);

        let scale = 1;
        if (projection === "linear" && count > 1) {
          scale = 1 - (v / (count + 1)) * 0.5;
        }

        let unit = sd * R.random_num(0.15, 0.35) * scale;

        // ================================================================
        // FRONT FACE CONSTRUCTION
        // ================================================================
        // Logo algorithm pattern: centrosymmetric quad with point symmetry
        // around (cx, cy). A and C mirror across center, B and D mirror.
        //
        // For Square: 4-corner quad — the most direct mapping from the logo.
        // For Circle: bounding ellipse centered at (cx, cy).
        // For Triangle: 3-vertex triangle in bounding box.
        // For Line: two endpoints spanning the front face.

        if (shape === "Square") {
          // --- Centrosymmetric quad (logo algorithm) ---
          let hw = unit * R.random_num(0.4, 1.0);
          let hh = unit * R.random_num(0.4, 1.0);
          // Slight asymmetry in B/D creates the irregular quad character
          let bShift = R.random_num(0.6, 1.0);
          let A1x = cx - hw, A1y = cy - hh;
          let B1x = cx - hw * bShift, B1y = cy + hh;
          let C1x = cx + hw, C1y = cy + hh;   // mirror of A
          let D1x = cx + hw * bShift, D1y = cy - hh;   // mirror of B

          // --- Project vertices (shape-independent helper) ---
          function projectVertex(vx, vy) {
            if (projection === "isometric") {
              return { x: vx + dx, y: vy + dy };
            } else {
              // Linear: vertex moves toward VP by depth fraction
              let t = depthMag / sd;
              return { x: vx + (dx - vx) * t, y: vy + (dy - vy) * t };
            }
          }

          let projA = projectVertex(A1x, A1y);
          let projB = projectVertex(B1x, B1y);
          let projC = projectVertex(C1x, C1y);
          let projD = projectVertex(D1x, D1y);

          // --- Face layers ---
          // Logo algorithm draws multiple face-pair selections per volume,
          // each with independent fill/stroke decisions. The face index (s)
          // determines which sides are rendered.
          //
          // s=1..4: side faces connecting front to back (top, left, bottom, right)
          // s=5..6: full back-plane quads
          //
          // For the rough: render all visible side faces, then front face on top.
          // Jeff refines face-pair selection and the "no repeat" constraint
          // (logo's prev != s logic) in his pass.

          let prev = 0;
          for (let f = 0; f < faceCount; f++) {
            // Color stepping (logo modColor pattern):
            // Desaturate/brighten across layers to create depth illusion.
            if (colorScheme === "stepped") {
              let t = (f + 1) / (faceCount + 2);
              let faceColor = betterLerp(c1, c2, t);
              fill(faceColor);
            ...
            // Face selection — which side faces to draw.
            // Avoid repeating the same face (logo's while s == prev pattern).
            let s = 0;
            while (s === prev) { s = R.random_int(1, 4); }

            if (s === 1) { // top face
              quad(A1x, A1y, projA.x, projA.y, projD.x, projD.y, D1x, D1y);
            }
            if (s === 2) { // left face
              quad(A1x, A1y, projA.x, projA.y, projB.x, projB.y, B1x, B1y);
            }
            if (s === 3) { // bottom face
              quad(B1x, B1y, projB.x, projB.y, projC.x, projC.y, C1x, C1y);
            }
            if (s === 4) { // right face
              quad(D1x, D1y, projD.x, projD.y, projC.x, projC.y, C1x, C1y);
            }
            prev = s;
          }

          // Front face on top — always drawn, with the terminal color step
          if (colorScheme === "stepped") {
            let t = (faceCount + 1) / (faceCount + 2);
            fill(betterLerp(c1, c2, t));
          } else {
            fill(c1);
          }
          if (outline) {
          ...
          quad(A1x, A1y, B1x, B1y, C1x, C1y, D1x, D1y);

        } else if (shape === "Circle") {
          // --- Cylinder ---
          // Front face: ellipse at (cx, cy) with semi-axes (hw, hh).
          // Back face: same ellipse projected by (dx, dy).
          // Side: two tangent lines connecting the ellipses.
          //
          // TODO: Jeff builds out the tangent-line math and face rendering.
          // The tangent points depend on the projection angle — for isometric,
          // they are the points on the ellipse where the tangent is parallel
          // to the projection vector.
          let hw = unit * R.random_num(0.5, 1.0);
          let hh = unit * R.random_num(0.5, 1.0);

          // Placeholder: draw front and back ellipses + connecting lines
          if (outline) {
            let sw = strokeWidth(sd, "medium");
            strokeWeight(sw); stroke(c1); noFill();
          } else {
            fill(c1); noStroke();
          }

          // Back ellipse (drawn first, behind)
          if (projection === "isometric") {
            ellipse(cx + dx, cy + dy, hw * 2, hh * 2);
          } else {
            let t = depthMag / sd;
            let bx = cx + (dx - cx) * t;
            let by = cy + (dy - cy) * t;
            let bScale = 1 - t * 0.3;
            ellipse(bx, by, hw * 2 * bScale, hh * 2 * bScale);
          }

          // Front ellipse
          if (colorScheme === "stepped") {
            fill(betterLerp(c1, c2, 0.7));
          }
          ellipse(cx, cy, hw * 2, hh * 2);

        } else if (shape === "Triangle") {
          // --- Triangular prism ---
          // Front face: triangle. Back face: projected triangle.
          // Sides: 3 parallelogram faces connecting front to back edges.
          let hw = unit * R.random_num(0.5, 1.0);
          let hh = unit * R.random_num(0.5, 1.0);
          // Equilateral-ish triangle centered at (cx, cy)
          let T1x = cx, T1y = cy - hh;           // apex
          let T2x = cx - hw, T2y = cy + hh;      // bottom-left
          let T3x = cx + hw, T3y = cy + hh;      // bottom-right

          function projectVert(vx, vy) {
            if (projection === "isometric") {
              return { x: vx + dx, y: vy + dy };
            } else {
              let t = depthMag / sd;
          ...
          let pT1 = projectVert(T1x, T1y);
          let pT2 = projectVert(T2x, T2y);
          let pT3 = projectVert(T3x, T3y);
          ...
          // Side faces
          for (let f = 0; f < Math.min(faceCount, 3); f++) {
            if (colorScheme === "stepped") {
              fill(betterLerp(c1, c2, (f + 1) / (faceCount + 2)));
            } else if (colorScheme === "binary") {
              fill(f % 2 === 0 ? c1 : c2);
            } else {
              fill(c1);
            ...
            if (f === 0) quad(T1x, T1y, pT1.x, pT1.y, pT2.x, pT2.y, T2x, T2y);
            if (f === 1) quad(T2x, T2y, pT2.x, pT2.y, pT3.x, pT3.y, T3x, T3y);
            if (f === 2) quad(T3x, T3y, pT3.x, pT3.y, pT1.x, pT1.y, T1x, T1y);
          }

          // Front triangle on top
          if (colorScheme === "stepped") {
            fill(betterLerp(c1, c2, (faceCount + 1) / (faceCount + 2)));
          } else { fill(c1); }
          if (outline) { noFill(); }
          ...
          // --- Extruded line ---
          // Front: a line segment. Back: its projected copy.
          // The "volume" is the parallelogram between them.
          let hw = unit * R.random_num(0.5, 1.2);
          // Line orientation
          let vertical = R.random_bool(0.5);
          let L1x, L1y, L2x, L2y;
          if (vertical) {
            L1x = cx; L1y = cy - hw;
            L2x = cx; L2y = cy + hw;
          ...
          let pL1 = projectPt(L1x, L1y);
          let pL2 = projectPt(L2x, L2y);

          // The extruded surface is a quad
          if (colorScheme === "stepped") {
            fill(betterLerp(c1, c2, 0.5));
          } else { fill(c1); }
          ...
          quad(L1x, L1y, pL1.x, pL1.y, pL2.x, pL2.y, L2x, L2y);

          // Front line on top
          strokeWeight(sw);
          stroke(colorScheme === "stepped" ? betterLerp(c1, c2, 0.8) : c1);
          line(L1x, L1y, L2x, L2y);
        }
      }

      print("Volume:", projection, "depth:", Math.round(depthMag),
            "angle:", depthAngle || "VP", "faces:", faceCount,
            "count:", count, "color:", colorScheme,
            outline ? "outlined" : "filled");
    }
  }
