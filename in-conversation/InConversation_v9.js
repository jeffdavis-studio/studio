...
let methods = {
  ...
  shapeProgression: {
    ...
    plan: function(shape, config) {
      ...
      let shrinkFactor;
      if (shape === "Triangle") {
        shrinkFactor = corner ? 1 / Math.sqrt(2) : 1 / Math.sqrt(5);
      } else {
      ...
      for (let i = 0; i < nt; i++) {
        ...
        if (shape === "Line") {
          ...
          if (corner) {
            ...
            // edge only (center folds into edge per alignmentOptions)
      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          if (outline) {
            ...
            fill(focused ? c1 : (muted ? betterLerp(c1, c2, 0.5) : cc));
          }
          ...
          if (isTarget && anomalyShape) {
            ...
            // form change. Outlined, sw goes along too: drawShape pulls the path in so the
            // outer edge of the stroke lands on that same footprint (see drawShape).
  ...
  // ---------------------------------------------------------------------------
  // DUO
  // Two forms placed together in one field, built to be compared rather than to fill the
  // canvas on their own — the method every other one here is missing for Contrast, which is
  // about a relationship between two things and has nothing to hold the second thing in.
  // Knobs: shapePair, sizeRatio, proximity, colorScheme, fillPair, outline, strokePair, baseline
  //   shapePair: "same" draws the rolled shape twice; "different" pairs it with one of the
  //     other three primitives this method supports. Direct route to Contrast > Shape.
  //     Line is a stroke of length `size` — the same measuring length as a square's side or a
  //     circle's diameter — standing across the baseline rather than along it, so two Lines sit
  //     side by side as parallels instead of end to end on one ray. Its footprint along the
  //     baseline is therefore its own weight, not `size`, and it turns with the pair, so it stays
  //     square to that line after a tilt. Triangle is equilateral with its height at `size`,
  //     stood in its bounding box rather than centered on its centroid — apex on the top edge,
  //     base on the bottom — so it spans the same top and bottom lines as a square or circle of
  //     the same size. Its base is 2/√3 of that, and the base is its footprint on the baseline.
  //   sizeRatio: "equal" gives both forms the same footprint; "contrast" scales one of them
  //     up by a ratio drawn from ratioChoices, with which of the two gets the larger footprint
  //     rolled separately from everything else — so a size contrast is never the same form
  //     that also happens to carry the shape or color contrast. Direct route to Contrast > Size.
  //   proximity: "gap" holds them apart by the canvas margin, "touching" brings them into
  //     contact at a literal zero, "overlap" lets them intrude on each other by a fraction of
  //     the smaller form. Touching and overlap both give way to a gap for the one pair that
  //     fuses on contact, described at the roll itself. Contrast > Position. Named proximity
  //     rather than
  //     spacing because grid and stripe already spend that word on even-vs-variable cell
  //     distribution, which is a different question with a different answer set — one word per
  //     meaning keeps a demand written for several methods at once from meaning two things.
  //   colorScheme: "single" is no color contrast, both forms c1 on the usual c2 ground.
  //     "pair" puts each parent on one form and the ground at the midpoint blend of the two —
  //     both parents visible, neither vanishing into the canvas. No other method has this
  //     relationship: their binary is c1/c2 with c2 as the ground (so a c2 entry is meant to
  //     vanish), and their gradient is a lerp that stops short of that same ground. Those
  //     names would lie here, so this one is its own. Contrast > Color.
  //   fillPair: "same" or "different" — solid vs outline. Two-stage like the baseline: a coin
  //     flip between same and different, then if same the existing outline chance paints both
  //     solid or both outlined. Different puts one of each, which form is outlined its own
  //     coin flip. A Line has no interior, so a pair that includes one cannot host a fill
  //     contrast and fillPair comes out "same". The outline knob is the pair-level reading of
  //     that same-fill case: true means both forms are strokes, false means both are solid.
  //     A pinned outline holds fillPair to "same", because mixed fill cannot satisfy a demand
  //     that both forms share one treatment.
  //   strokePair: "same" or "different" only for two Lines. Two-stage. Different picks two
  //     distinct names from the shared thin-end catalog, so the step is a named one (medium
  //     against fine, not a continuous roll that sometimes comes back almost equal). Two
  //     outlined closed forms, or a Line against an outlined form, share one weight and
  //     report "same". "none" when only one form is stroked. Contrast of weight, and only
  //     Line against Line.
  //   baseline: the tilt of the line both forms sit on, in degrees: "0", "30", "45", "60".
  //     The roll is two-stage, not a flat pick from four: first a coin flip between zero and
  //     rotated, then if rotated one of 30, 45, 60. That keeps the horizontal pair — the reading
  //     that lets a size or shape difference sit as the point of the image — as likely as a
  //     tilt, instead of outnumbered three to one. A tilt turns the line under the pair as a
  //     rigid body, so proximity is the same relationship it was. The sign of the tilt is its
  //     own coin flip and is not reported: 30° clockwise and 30° counter are the same demand,
  //     and the canvas quarter-turns in draw() already send each of those into the other three
  //     quadrants. Contrast > Position.
  //   ratioChoices: doubled or tripled, the same two steps every other method makes a size
  //     difference in — shapeGrid's scale spans a 2x2 or 3x3 block of cells, stripe's widens one
  //     band by a fixed 3x, and largeShape works in whole lattice units. Held to integers on
  //     stripe's reasoning for its own factor, that doubling is the smallest departure that
  //     reads as deliberate: anything under it comes back almost equal and says nothing, which
  //     is the one thing a contrast cannot afford to do.
  //   overlapChoices: how far the forms intrude on each other, as a fraction of the smaller
  //     one — shapeGrid's concSparse states its gap the same way, against the size of a shape
  //     rather than of the canvas, so the figure means the same thing at any scale. Always
  //     half: a quarter sat closer to touching than to a second reading, and an eighth
  //     closer still. The pool is one value so a subtopic can still narrow or replace it.
  //   These pools are held out here rather than inline for the same reason as elementChoices
  //     and stripeChoices — a subtopic that wants only the emphatic end of a range narrows the
  //     pool instead of rolling the full one and rejecting most of what comes back. A gap
  //     needs no pool at all: it is the canvas margin, on shapeGrid's rule that an axis spaces
  //     its cells at the same interval it insets them from the edge.
  //
  // Placement solves one scale S for the pair: each form's size is its share of S (the split
  // derived from sizeRatio), and the pair is sized so the frame holds it on both axes. Along the
  // baseline the two footprints plus the space between them (derived from the proximity knob)
  // fill the inset frame exactly; across it, the taller form stops at the same frame. A closed
  // form's footprint is its size, so for those pairs the baseline is what binds, and the solve
  // is the same margins + cells + spacing = canvas that solveAxis does for grid, shapeGrid and
  // stripe. It is not handed to solveAxis because a Line breaks the proportionality that
  // function assumes: its footprint along the baseline is its stroke weight, which follows the
  // smaller form rather than its own share, and a pair of Lines is bound by its height instead.
  // The canvas margin comes from the same small pool grid and stripe draw their own margins
  // from. There is no absolute-size knob, no failure case where the two do not fit, and no
  // separate lattice.
  //
  // Stroke weight is referenced to the smaller of the two footprints, per the universal policy:
  // cell size for the methods that place cells, canvas size only for the ones whose form spans
  // the canvas. These forms never do — they are a third to two thirds of it — so a weight read
  // off the canvas would put the same stroke on a small form as on a large one and, on the small
  // end, a rule nearly a fifth of the form's own width. The smaller footprint rather than each
  // form's own is because a named contrast has to be a named contrast: both names are read
  // off the same unit, so medium against fine stays that step even when the forms themselves
  // differ in size. Only two Lines take that contrast. An outlined closed form still has
  // its path pulled in by half its weight so the outer edge of the stroke lands on the
  // same footprint a fill of that size occupies — the same ink-on-the-box rule largeShape
  // holds, and the only way a solid-vs-outline pair compares treatment without also
  // comparing size.
  //
  // The pair is always built left to right. Fine tilts live on the baseline knob; the shared
  // canvas rotation in draw() still supplies the four quadrants, the same source every other
  // method here gets its axis variety from.
  //
  // No range or fit knob either, the one piece of the shared vocabulary this method leaves out:
  // every other method has some say in how it meets the canvas edge, and this one is always
  // inset on both ends. Two forms being compared need a frame to be compared inside — run to
  // the edge or off it and the reading changes from "these two against each other" to "this one
  // against the canvas," which is what largeShape is for.
  duo: {
    shapes: ["Line", "Circle", "Square", "Triangle"],
    defaults: {
      shapePair: "random",
    ...
    // Unwired. Built for Contrast — Shape, Size, Color and Position all read directly off its
    // knobs — but that topic has no wiring yet at all, this method included. Reachable through
    // test mode meanwhile.
    subtopics: {},
    plan: function(shape, config) {
      let shapePair = resolveChoice(config.shapePair, ["same", "different"]);
      let otherShapes = methods.duo.shapes.filter(function(sh) { return sh !== shape; });
      // "different" always has somewhere to go — the method holds four primitives — but
      // the fallback is here anyway so a future narrowing of this method's shape list fails
      // honestly (shapePair reported as "same") rather than by drawing two identical forms
      // under a manifest that still claims "different".
      let shape2 = (shapePair === "different" && otherShapes.length > 0)
        ? R.random_choice(otherShapes) : shape;
      if (otherShapes.length === 0) shapePair = "same";

      let colorScheme = resolveChoice(config.colorScheme, ["single", "pair"]);
      // pair: each form is one parent, the ground is their midpoint. Which form gets which
      // parent is its own coin flip, so a color contrast lands on the larger form and the
      // smaller one equally often — the shape, size, and color axes stay legible apart from
      // each other instead of stacking up on one. single leaves the canvas as c2, the same
      // ground every other method uses.
      let palette, ground;
      if (colorScheme === "pair") {
        palette = R.random_bool(0.5) ? [c1, c2] : [c2, c1];
        ground = betterLerp(c1, c2, 0.5);
      } else {
        palette = [c1, c1];
        ground = null;
      }

      let sizeRatio = resolveChoice(config.sizeRatio, ["equal", "contrast"]);
      let ratio = sizeRatio === "contrast" ? R.random_choice(config.ratioChoices) : 1;
      let firstLarger = R.random_bool(0.5);

      let proximity = resolveChoice(config.proximity, ["gap", "touching", "overlap"]);
      let tiltOptions = ["30", "45", "60"];
      let baselineOptions = ["0"].concat(tiltOptions);
      let baseline;
      if (config.baseline === "random" || config.baseline === undefined || config.baseline === null) {
        baseline = R.random_bool(0.5) ? "0" : R.random_choice(tiltOptions);
      } else {
        baseline = resolveChoice(config.baseline, baselineOptions);
      }
      let tilt = parseInt(baseline, 10);
      // Clockwise or counter is not a different reading of Position, so it is not a knob.
      let tiltSign = R.random_bool(0.5) ? 1 : -1;

      // --- Fill and stroke ---
      // A Line has no interior, so it cannot take part in a solid-vs-outline contrast: the
      // pair comes out "same" and says so. A pinned outline (true or false) is a demand on
      // both forms, which is the same-fill reading, so it too holds fillPair to "same".
      // Weight contrast is only Line against Line. Two outlined closed forms share one
      // named weight; that is a style, not a contrast.
      let canMixFill = shape !== "Line" && shape2 !== "Line";
      let outlinePinned = typeof config.outline === "boolean";
      let fillPair;
      if (config.fillPair === "random" || config.fillPair === undefined || config.fillPair === null) {
        fillPair = (canMixFill && !outlinePinned && R.random_bool(0.5)) ? "different" : "same";
      } else {
        fillPair = resolveChoice(config.fillPair, ["same", "different"]);
        if (fillPair === "different" && !canMixFill) fillPair = "same";
      }
      let outline1, outline2;
      if (fillPair === "different") {
        outline1 = R.random_bool(0.5);
        outline2 = !outline1;
      } else {
        let both = chance(config.outline);
        outline1 = both;
        outline2 = both;
      }
      if (shape === "Line") outline1 = true;
      if (shape2 === "Line") outline2 = true;
      let outline = fillPair === "same" && outline1 && outline2;

      let bothStroked = outline1 && outline2;
      let bothLines = shape === "Line" && shape2 === "Line";
      let strokePair;
      if (!bothStroked) {
        strokePair = "none";
      } else if (!bothLines) {
        strokePair = "same";
      } else if (config.strokePair === "random" || config.strokePair === undefined || config.strokePair === null) {
        strokePair = R.random_bool(0.5) ? "same" : "different";
      } else {
        strokePair = resolveChoice(config.strokePair, ["same", "different"]);
      }

      // Contact has to leave the two forms readable as two. The pairs it cannot are two filled
      // squares, same tone, and two lines, same tone and same weight: both meet on a shared
      // edge that is identically inked, so they fuse into a single silhouette — one
      // rectangle, or one thicker rule — whether they meet at a seam or run into each other.
      // A tilt does not save them: the pair turns as a rigid body, so the join is the same
      // join it was. Two lines of different weight still meet, but the join is a change of
      // thickness, which is a contrast rather than a fuse. Everything else reads correctly —
      // a circle meets its neighbor at a tangent and a triangle on a 60° edge, both of which
      // leave background between the forms on either side of the meeting point; two tones
      // show the seam as a change of color; and two outlined squares meet at their outer
      // edges, each form keeping its own rule, which is how two outlined squares sit.
      //
      // Where it does fuse, the state is the wrong one for the pair that rolled rather than
      // something to paper over with clearance — a hairline of relief only trades the fused
      // reading for a gap too small to be a gap — so proximity gives way to a gap, and says so.
      // It is a narrow case, and a pinned touching lands on a pair that can show it within an
      // attempt or two.
      //
      // Two Lines have no overlap to offer in the first place. Parallel strokes intruding on each
      // other put one inside the other's width, which reads as a single rule with a step in its
      // weight — what contact already shows, only less cleanly. So overlap gives way to touching
      // for them, ahead of the fuse rule, which then decides whether touching itself can stand.
      if (proximity === "overlap" && bothLines) proximity = "touching";
      if (proximity !== "gap" && colorScheme === "single" && shape === shape2) {
        if (shape === "Square" && !outline1 && !outline2) proximity = "gap";
        if (shape === "Line" && strokePair !== "different") proximity = "gap";
      }

      // --- Solve the sizes ---
      // Every form is measured by one `size` — a square's side, a circle's diameter, a
      // triangle's height, a line's length — the same convention largeShape uses for its
      // driving axis, so a size ratio compares like to like.
      //
      // margin: the pool shapeGrid and stripe both draw their canvas margins from, under
      // shapeGrid's own bound on it. A layout of n cells inset on both sides spends (n+1)·v of
      // the span on margins and gaps, and requiring the cells to come out at least as wide as
      // that spacing gives v·(2n+1) ≤ sd. Here n is 2 and the number of intervals is what
      // proximity decides — one when it asked for a gap, none when the forms meet — which works
      // out to v ≤ sd/5 for a gap and v ≤ sd/4 otherwise. So a gap spends sd/4 out of the pool
      // and leaves the two values grid uses, while touching and overlap keep all three and can
      // hold the pair small in a wide field.
      let intervals = proximity === "gap" ? 1 : 0;
      let marginPool = [sd / 16, sd / 8, sd / 4].filter(v => v * (4 + intervals) <= sd);
      let marginPick = R.random_choice(marginPool);
      let availW = sd - 2 * marginPick;

      // Each form's share of the pair's scale, ordered to match firstLarger so props[0] always
      // lands on `shape`'s form and props[1] on shape2's.
      let pSmall = 1 / (1 + ratio), pLarge = ratio / (1 + ratio);
      let props = firstLarger ? [pLarge, pSmall] : [pSmall, pLarge];

      // --- Stroke weight (proportional to the smaller form) ---
      // The named set is the thin end of the catalog. A weight is a rule around a form here,
      // not a grid line dividing cells, and these forms are large: the heavier divisors that
      // read as structure at cell scale would read as an annulus at this one. The ratio
      // thresholds never bite at this scale, but the selection goes through the shared helper
      // anyway, so a future narrowing of the sizes is caught by the same rule every other method
      // is held to.
      // The names are settled before the sizes, because a Line's weight is its footprint and so
      // feeds the solve below. They are read off the smaller form as the solve would size it
      // with every Line counted at its full size across the baseline, which is a floor on what
      // the solve delivers — a Line's real footprint is narrower than that, and a pair's height
      // only binds above it — and the thresholds only ever admit more names as a form grows, so
      // no name picked here can come out invalid at the solved size.
      let reach = function(sh, p, name) {
        if (sh === "Line") return name ? pSmall / STROKE_WEIGHTS[name] : p;
        return duoFootprint(sh, p, 0);
      };
      let unit = pSmall * (availW - (proximity === "gap" ? marginPick : 0))
        / (reach(shape, props[0], null) + reach(shape2, props[1], null));
      let swNames = pickStrokeWeights(["medium"], unit / sd);
      // One unit for both forms, even when the names differ: a size contrast already changes
      // how much of the form a given weight occupies, and stacking that under a named contrast
      // would make the name the quieter of the two readings. No gap filter: the stroke sits
      // inside the box, so a weight cannot close a space the proximity roll asked for.
      if (strokePair === "different" && swNames.length < 2) strokePair = "same";
      let pickName = function(avoid) {
        let pool = avoid ? swNames.filter(function(n) { return n !== avoid; }) : swNames;
        return R.random_choice(pool.length > 0 ? pool : swNames);
      };
      let swName1 = outline1 ? pickName(null) : null;
      let swName2 = outline2
        ? (strokePair === "different" ? pickName(swName1) : (swName1 || pickName(null)))
        : null;
      let widthOf = function(name) {
        if (!name) return 0;
        return strokeWidth(unit, name);
      };

      // --- Solve the pair's scale ---
      // One scale, ps, for the pair: each form's size is its share of it. Its footprint along the
      // baseline (see duoFootprint) is that size for a square or circle, 2/√3 of it for a
      // triangle's base, and its stroke weight for a Line — a weight read off the smaller form,
      // so that too is a fixed fraction of ps. Along the baseline the two
      // footprints and the space between them fill the frame exactly; across it, the taller
      // form stops at the frame. Whichever binds first sets ps. For two closed forms that is
      // always the baseline, and this is the same margins + cells + spacing = canvas that
      // solveAxis does for the grid methods. A Line gives most of its share of the baseline
      // back, so a pair with one in it grows until the frame's height or the baseline catches
      // it, and a pair of Lines is always held by its height.
      let across = reach(shape, props[0], swName1) + reach(shape2, props[1], swName2);

      // sp: the space between the two footprints.
      //
      // A gap is the margin, which is shapeGrid's rule exactly — its per-axis spacing is that
      // axis's inset baseline, so that "canvas edge → first cell" and "cell → cell" are the same
      // interval and the field keeps one rhythm. Read on a pair that is the plainest case of it:
      // the space between the two forms is the space around them. It also means a gap needs no
      // pool of its own. Outlined ink stays inside each form's box, so a weight never eats
      // the interval the proximity roll asked for.
      //
      // Overlap is the one figure left to name, and it is named the way shapeGrid's concSparse
      // names its own: as a ratio against the size of a shape rather than a fraction of the
      // canvas, so it reads as the same intrusion whatever the pair's scale. Half of the
      // smaller form — the one step of that ladder that still reads as an intrusion. The
      // smaller form is a fixed fraction of ps like everything else here, so the intrusion is
      // solved exactly along with it: negative space between the footprints, which the scale
      // grows to cover so the pair still spans the frame.
      let overlap = proximity === "overlap" ? R.random_choice(config.overlapChoices) : null;
      let ps;
      if (proximity === "touching") {
        ps = availW / across;
      } else if (proximity === "overlap") {
        ps = availW / (across - overlap * pSmall);
      } else {
        ps = (availW - marginPick) / across;
      }
      ps = Math.min(ps, availW / pLarge);

      let size1 = props[0] * ps, size2 = props[1] * ps;
      unit = pSmall * ps;
      let sw1 = widthOf(swName1), sw2 = widthOf(swName2);
      let sp = proximity === "touching" ? 0
        : proximity === "overlap" ? -overlap * unit
        : marginPick;
      let fw1 = duoFootprint(shape, size1, sw1);
      let fw2 = duoFootprint(shape2, size2, sw2);
      // Centered on the baseline: flush to the frame at both ends when the baseline is what
      // bound, and with equal room either side when the height did.
      let x1 = (sd - fw1 - fw2 - sp) / 2;
      let x2 = x1 + fw1 + sp;
      let cy = sd / 2;
      // Touching is an ink relationship. Square and line fill their footprint top to bottom, so
      // footprint contact is ink contact. A triangle reaches its footprint's sides only at its
      // base corners and a circle only at its middle, so where the neighbor is a different
      // height those points can pass it by and footprint contact leaves a gap. Nudge the second
      // form along the baseline until the interiors just stop intersecting.
      if (proximity === "touching") {
        let y1 = cy - size1 / 2, y2 = cy - size2 / 2;
        x2 += shiftToTouch(shape, x1, y1, size1, outline1 ? sw1 : 0,
          shape2, x2, y2, size2, outline2 ? sw2 : 0);
      }

      // A tilt turns the finished pair as a rigid body about the canvas center, so the
      // relationship the solve just settled — sizes, gap, contact — is the same relationship
      // after the turn. The axis-aligned box of that turned pair is larger than the pair was,
      // and at 45° it is larger by √2 on the corners; scaling toward the center by whatever
      // keeps that box inside the same inset frame is what lets 45° exist without hanging off
      // the canvas or inventing a second, smaller layout.
      // Fit against the ink rather than the footprint, since a triangle's corners are all that
      // reach the corners of its footprint and its slanted sides turn well inside them.
      let boxes = [
        { shape: shape, x: x1, y: cy - size1 / 2, w: fw1, s: size1 },
        { shape: shape2, x: x2, y: cy - size2 / 2, w: fw2, s: size2 }
      ];
      let deg = tilt * tiltSign;
      let rad = deg * Math.PI / 180;
      let cosT = Math.cos(rad), sinT = Math.sin(rad);
      let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
      for (let b = 0; b < boxes.length; b++) {
        let bx = boxes[b].x, by = boxes[b].y, bw = boxes[b].w, bs = boxes[b].s;
        let corners = boxes[b].shape === "Triangle"
          ? boxTriangleVerts(bx, by, bs)
          : [[bx, by], [bx + bw, by], [bx, by + bs], [bx + bw, by + bs]];
        for (let k = 0; k < corners.length; k++) {
          let dx = corners[k][0] - sd / 2, dy = corners[k][1] - sd / 2;
          let rx = sd / 2 + dx * cosT - dy * sinT;
          let ry = sd / 2 + dx * sinT + dy * cosT;
          minX = Math.min(minX, rx); maxX = Math.max(maxX, rx);
          minY = Math.min(minY, ry); maxY = Math.max(maxY, ry);
        }
      }
      // Ink stays inside these verts — an outlined path is inset by half its weight, and a Line's
      // footprint is its own stroke — so the fit is against the same geometry a fill of this
      // size occupies, not a stroke halo around it.
      let limit = sd / 2 - marginPick;
      let ext = Math.max(sd / 2 - minX, maxX - sd / 2, sd / 2 - minY, maxY - sd / 2);
      let fit = ext > limit && ext > 0 ? limit / ext : 1;
      if (fit < 1) {
        // A footprint's center moves toward the canvas center and its width shrinks with it. A
        // Line's width is its weight, which the new unit shrinks by the same factor.
        let scaleAboutCenter = function(v, w) {
          return sd / 2 + fit * (v + w / 2 - sd / 2) - fit * w / 2;
        };
        x1 = scaleAboutCenter(x1, fw1);
        x2 = scaleAboutCenter(x2, fw2);
        size1 *= fit;
        size2 *= fit;
        unit = Math.min(size1, size2);
        sw1 = widthOf(swName1);
        sw2 = widthOf(swName2);
      }

      let forms = [
        { shape: shape, x: x1, y: cy - size1 / 2, size: size1, color: palette[0],
          outline: outline1, sw: sw1, swName: swName1 },
        { shape: shape2, x: x2, y: cy - size2 / 2, size: size2, color: palette[1],
          outline: outline2, sw: sw2, swName: swName2 }
      ];

      return {
        manifest: {
          shapePair: shapePair, sizeRatio: sizeRatio, proximity: proximity,
          colorScheme: colorScheme, fillPair: fillPair, outline: outline,
          strokePair: strokePair, baseline: baseline
        },
        state: {
          shape2: shape2, forms: forms, outline: outline,
          fillPair: fillPair, strokePair: strokePair,
          shapePair: shapePair, sizeRatio: sizeRatio, ratio: ratio, firstLarger: firstLarger,
          proximity: proximity, overlap: overlap, colorScheme: colorScheme, baseline: baseline,
          tilt: deg, ground: ground
        }
      };
    },
    render: function(p) {
      let { shape2, forms, fillPair, strokePair, shapePair, sizeRatio, ratio, firstLarger,
            proximity, overlap, colorScheme, baseline, tilt, ground } = p;

      print("Shapes:", shapePair === "different" ? shape + " + " + shape2 : shape + " ×2");
      print("Size ratio:", sizeRatio === "contrast"
        ? ratio.toFixed(2) + "x, " + (firstLarger ? "first" : "second") + " larger"
        : "equal");
      if (overlap != null) {
        let frac = overlap === 1 / 2 ? "1/2" : overlap.toFixed(2);
        print("Proximity:", proximity, "| " + frac + " of smaller");
      } else {
      ...
      print("Fill:", fillPair);
      if (strokePair !== "none") {
        print("Stroke:", strokePair === "different"
          ? forms[0].swName + " / " + forms[1].swName
          : (forms[0].swName || forms[1].swName));
      } else if (forms[0].outline || forms[1].outline) {
        print("Stroke:", forms[0].swName || forms[1].swName);
      }

      if (ground) background(ground);
      ...
      for (let i = 0; i < forms.length; i++) {
        let f = forms[i];
        if (f.shape === "Line") {
          // No interior to fill — always a stroke, standing across the baseline at the middle
          // of its footprint, which is its own weight wide. The global PROJECT cap extends drawn
          // ink by sw/2 past each endpoint; insetting the segment by that pad keeps the ink
          // inside the [y, y+size] span the solve measured, so the line is its size long.
          noFill();
          stroke(f.color);
          strokeWeight(f.sw);
          let cap = Math.min(f.sw / 2, f.size / 2);
          let lx = f.x + f.sw / 2;
          line(lx, f.y + cap, lx, f.y + f.size - cap);
        } else {
          if (f.outline) {
            noFill();
            stroke(f.color);
          ...
          if (f.shape === "Triangle") {
            // Stood in its own box rather than centered on its centroid (see boxTriangleVerts).
            // Outlined, the path comes in by half the weight so the outer edge of the ink, miters
            // included, lands on the filled triangle, as drawShape does for the other methods.
            let v = boxTriangleVerts(f.x, f.y, f.size);
            if (f.outline) {
              drawingContext.miterLimit = 1000;
              v = insetConvex(v, Math.min(f.sw / 2, f.size / 6));
            }
            triangle(v[0][0], v[0][1], v[1][0], v[1][1], v[2][0], v[2][1]);
          } else {
            drawShape(f.shape, f.x, f.y, f.size, f.size, f.outline ? f.sw : 0);
          }
        }
      }
      pop();
...
const SUBTOPIC_PARAMS = [
  ...
  "insetMinU", "insetMaxU", "ratioChoices", "overlapChoices"
];
...
// Draw one primitive inside the box [x, y, w, h]. Square spans the box. Circle is the
// inscribed circle of diameter min(w,h). Triangle is the equilateral of that same height —
// so a triangle, a square, and a circle of one `size` are the same tall — with its centroid
// (which is also its circumcenter) on the box center, apex up. Height = circumradius × 3/2,
// so the circumradius is 2/3 of the box, not half: the apex sits above the box and the base
// is wider than the box (side = 2/√3 of the height). That overflow is real; callers that
// need the ink's box ask shapeBox. duo stands its triangles in their box instead — see
// boxTriangleVerts.
//
// `sw` is optional and only matters when the caller is going to stroke the result. p5 centers a
// stroke on its path, so a path drawn on the filled outline would put its outer ink sw/2 past
// that outline — a larger form than the filled one of the same box. Passing sw pulls the path
// in by sw/2 (and, for a triangle, through insetConvex so the miters come with it) and lands
// the OUTER edge of the stroke on the filled footprint. A square's right-angle miters reach
// exactly sw/2 on each axis and a circle has none, so both stop at the box on their own;
// a triangle's 60° corners would spike well past it without the inset. Omit sw for fills.
function equilateralVerts(cx, cy, height) {
  let cr = 2 * height / 3;
  let v = [];
  for (let i = 0; i < 3; i++) {
    let a = -Math.PI / 2 + i * (2 * Math.PI / 3);
    v.push([cx + cr * Math.cos(a), cy + cr * Math.sin(a)]);
  }
  return v;
}

// duo's triangle: the equilateral of height `height` standing in its own bounding box, whose
// top-left is (x, y) — apex on the top edge, base on the bottom, so it is exactly as tall as a
// square or circle of the same size and sits on the same top and bottom lines they do. Its width
// is 2/√3 of that height, and that width is the footprint duo lays it out by.
function boxTriangleVerts(x, y, height) {
  let tw = height * 2 / Math.sqrt(3);
  return [[x + tw / 2, y], [x + tw, y + height], [x, y + height]];
}

// How much of duo's baseline a form takes up: a square's side, a circle's diameter, a triangle's
// base, and a Line's own weight, since it stands across the baseline rather than along it.
function duoFootprint(shape, size, sw) {
  if (shape === "Line") return Math.max(sw, 1);
  if (shape === "Triangle") return size * 2 / Math.sqrt(3);
  return size;
}

function inkPoly(shape, x, y, size, sw) {
  if (shape === "Square") {
    return [[x, y], [x + size, y], [x + size, y + size], [x, y + size]];
  }
  if (shape === "Triangle") return boxTriangleVerts(x, y, size);
  if (shape === "Line") {
    // Standing across the baseline: its own weight wide, its size tall.
    let t = Math.max(sw, 1);
    return [[x, y], [x + t, y], [x + t, y + size], [x, y + size]];
  }
  return null;
}

function satInteriorsOverlap(A, B) {
  let polys = [A, B];
  for (let p = 0; p < 2; p++) {
    let P = polys[p];
    for (let i = 0; i < P.length; i++) {
      let j = (i + 1) % P.length;
      let nx = P[j][1] - P[i][1], ny = P[i][0] - P[j][0];
      let minA = Infinity, maxA = -Infinity, minB = Infinity, maxB = -Infinity;
      for (let k = 0; k < A.length; k++) {
        let d = A[k][0] * nx + A[k][1] * ny;
        if (d < minA) minA = d;
        if (d > maxA) maxA = d;
      }
      for (let k = 0; k < B.length; k++) {
        let d = B[k][0] * nx + B[k][1] * ny;
        if (d < minB) minB = d;
        if (d > maxB) maxB = d;
      }
...
function pointInConvex(px, py, v) {
  ...
  for (let i = 0; i < v.length; i++) {
    ...
    if (c !== 0) {
      ...
      else if ((c > 0 ? 1 : -1) !== sign) inside = false;
    }
  }
  return inside;
}

// A circle meets a polygon when its center is inside it or an edge passes within its radius.
// Both halves are needed: a small circle wholly inside a large square is nowhere near an edge.
function circleHitsPoly(cx, cy, r, v) {
  if (pointInConvex(cx, cy, v)) return true;
  for (let i = 0; i < v.length; i++) {
    if (distToSeg(cx, cy, v[i], v[(i + 1) % v.length]) < r) return true;
  }
  return false;
}

function inksOverlap(sh1, x1, y1, s1, sw1, sh2, x2, y2, s2, sw2) {
  if (sh1 === "Circle" && sh2 === "Circle") {
    return Math.hypot(x1 + s1 / 2 - (x2 + s2 / 2), y1 + s1 / 2 - (y2 + s2 / 2)) < (s1 + s2) / 2;
  }
  if (sh1 === "Circle") {
    return circleHitsPoly(x1 + s1 / 2, y1 + s1 / 2, s1 / 2, inkPoly(sh2, x2, y2, s2, sw2));
  }
  if (sh2 === "Circle") {
    return circleHitsPoly(x2 + s2 / 2, y2 + s2 / 2, s2 / 2, inkPoly(sh1, x1, y1, s1, sw1));
  }
  return satInteriorsOverlap(inkPoly(sh1, x1, y1, s1, sw1), inkPoly(sh2, x2, y2, s2, sw2));
}

// How far to move form B along x so the two interiors just stop overlapping. Searched between B
// centered on A, where two forms centered on one baseline always overlap, and B's footprint
// starting where A's ends, where the ink cannot, since every form's ink stays inside its
// footprint. The forms are convex, so the offsets where they overlap form one unbroken run, and
// the search closes on its end.
function shiftToTouch(sh1, x1, y1, s1, sw1, sh2, x2, y2, s2, sw2) {
  let fw1 = duoFootprint(sh1, s1, sw1);
  let fw2 = duoFootprint(sh2, s2, sw2);
  let lo = x1 + fw1 / 2 - fw2 / 2;
  let hi = x1 + fw1;
  for (let i = 0; i < 28; i++) {
    let mid = (lo + hi) / 2;
    if (inksOverlap(sh1, x1, y1, s1, sw1, sh2, mid, y2, s2, sw2)) lo = mid;
    else hi = mid;
  }
  return hi - x2;
}
...
function drawShape(shape, x, y, w, h, sw) {
  ...
  let d = sw > 0 ? Math.min(sw / 2, u / 2) : 0;
  if (shape === "Circle") ellipse(cx, cy, u - 2 * d, u - 2 * d);
  else if (shape === "Square") rect(x + d, y + d, w - 2 * d, h - 2 * d);
  else if (shape === "Triangle") {
    let v = equilateralVerts(cx, cy, u);
    if (d > 0) {
      drawingContext.miterLimit = 1000;
      v = insetConvex(v, Math.min(d, u / 6));
    }
    triangle(v[0][0], v[0][1], v[1][0], v[1][1], v[2][0], v[2][1]);
  }
}
...
// The measuring box a shape occupies inside the box it was given, as [x, y, w, h]. Square
// spans its box; Circle and Triangle share the inscribed min(w,h) square — the circle fills
// that square, the triangle is the same height and overflows it. Callers that swap primitives
// in place draw the replacement into this square, which keeps the swap a change of form
// rather than of size.
function shapeBox(shape, x, y, w, h) {
  if (shape !== "Circle" && shape !== "Triangle") return [x, y, w, h];
  let u = Math.min(w, h);
...
// duo at full random, the method just added.
let testCases = [
  { methods: ["duo"], config: {} }
];
