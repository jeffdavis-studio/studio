...
// --- largeShape lattice constants + per-shape regularity vocabulary ---
// largeShape draws on "graph paper": a square lattice of LARGE_SHAPE_GRID modules per
// canvas side. Every vertex / center / radius lands on a lattice node (integer module
// units), so orientation is emergent from lattice edge vectors rather than an applied
// rotation. LARGE_SHAPE_REG_META is just the per-shape list of regularity subtypes; the
// payloads are intentionally empty — each subtype's geometry is rolled in buildFigure.
const LARGE_SHAPE_GRID = 8;

const LARGE_SHAPE_REG_META = {
  Line:     { "orthogonal": {}, "diagonal": {} },
  Circle:   { "circle": {}, "ellipse": {} },
  Square:   { "rectangle": {}, "square": {}, "tilted-square": {}, "parallelogram": {}, "trapezoid": {}, "irregular-quad": {} },
  Triangle: { "right": {}, "isoceles": {}, "irregular": {} }
};
...
let methods = {
  ...
  // ---------------------------------------------------------------------------
  // LARGE SHAPE
  // One (or two) canvas-scale shapes drawn on "graph paper": a square lattice of
  // LARGE_SHAPE_GRID modules per canvas side. The shape's identity comes first (Line /
  // Circle / Square / Triangle, with shape-specific regularity subtypes); its geometry is
  // rolled directly on lattice nodes (integer module coordinates) and then translated so its
  // centroid lands in-canvas. Orientation is emergent from lattice edge vectors — there is
  // no per-shape rotation resolver or bbox/free placement resolver anymore.
  //
  ...
  //   regularity: per-shape subtype selecting which lattice figure gets built:
  //       Line:     orthogonal (H/V) | diagonal (tilted node-to-node run)
  //       Circle:   circle (equal radii) | ellipse (unequal radii, axis-aligned)
  //       Square:   rectangle | square | tilted-square | parallelogram | trapezoid |
  //                 irregular-quad
  //       Triangle: right | isoceles | irregular
  //     tilted-square (and parallelogram) build from a lattice edge vector v=(a,b) and its
  //     perpendicular (-b,a), so the tilt is exact with integer vertices. Exact equilateral
  //     triangles and tilted ellipses aren't representable on a square lattice and are omitted.
  //   Orientation: no per-shape rotation. Variety comes from emergent lattice tilt plus the
  //     global canvas rotate(0/90/180/270) applied once in draw().
  //   Sizing / placement: sizes are integer module counts (canvas-scale); the figure is
  //     integer-translated so its centroid lands inside the canvas, while vertices may spill
  //     into the surrounding band (the cropped-field look). Containment is true by
  //     construction — no shrink loop. A filled figure that would cover all four canvas
  //     corners (a solid-canvas degeneracy) is nudged off one corner.
  //
  //   second-shape (pair) knobs — drawing two shapes joined by module-cell adjacency:
  //     secondShape: probability of drawing a second shape.
  //     pairMode: "identical" → shape B is a congruent copy of shape A; "varied" → shape B
  //       re-rolls its own size within shape A's regularity.
  //     pairRelation: "touching" → shape B's module bbox abuts shape A's with zero gap;
  //       "floating" → plus a 1-2 module gap. Forced to "floating" for Line (two segments
  //       meeting end-to-end would read as one bent line).
  //     contactStyle: "edge" → cardinal (face) adjacency; "corner" → diagonal adjacency.
  //       Only polygons have flat faces; ellipse/line fall back to "corner".
  //     anomaly: shape B can read as a deliberate "emphasis" outlier (same concept as
  //       grid/shapeGrid/stripe): outlined → B filled solid with c1 (silhouette); filled →
  //       B's fill becomes the c1↔c2 midpoint. "none" → B matches A.
  // ---------------------------------------------------------------------------
  largeShape: {
    shapes: ["Line", "Circle", "Square", "Triangle"],
    defaults: {
    ...
    draw: function(shape, config) {
      ...
      // Whether to draw a second shape (pair composition).
      let secondShape = chance(config.secondShape);

      // === Lattice substrate ===
      // Graph-paper grid: G modules per canvas side, modPx pixels each. Figures are built
      // and composed in integer module coordinates, then scaled to pixels at render time.
      const G = LARGE_SHAPE_GRID;
      let modPx = sd / G;
      let rint = function(a, b) { return R.random_int(a, b); };

      // --- Geometry helpers (module space) ---
      // Figures are { kind, verts:[[i,j],...] } for line/poly3/poly4, or
      // { kind:"ellipse", cx, cy, rx, ry } — all in integer module units, plus an `info`
      // field carrying printable subtype detail.
      let vertsOf = function(g) {
        if (g.kind === "ellipse")
          return [[g.cx - g.rx, g.cy - g.ry], [g.cx + g.rx, g.cy - g.ry],
                  [g.cx + g.rx, g.cy + g.ry], [g.cx - g.rx, g.cy + g.ry]];
        return g.verts;
      };
      let bboxOf = function(g) {
        let vs = vertsOf(g);
        let xs = vs.map(function(v) { return v[0]; });
        let ys = vs.map(function(v) { return v[1]; });
        return { minx: Math.min.apply(null, xs), maxx: Math.max.apply(null, xs),
                 miny: Math.min.apply(null, ys), maxy: Math.max.apply(null, ys) };
      };
      let centroidOf = function(g) {
        if (g.kind === "ellipse") return [g.cx, g.cy];
        let sx = 0, sy = 0;
        for (let i = 0; i < g.verts.length; i++) { sx += g.verts[i][0]; sy += g.verts[i][1]; }
        return [sx / g.verts.length, sy / g.verts.length];
      };
      let translateGeom = function(g, dx, dy) {
        if (g.kind === "ellipse") { g.cx += dx; g.cy += dy; }
        else for (let i = 0; i < g.verts.length; i++) { g.verts[i][0] += dx; g.verts[i][1] += dy; }
        return g;
      };
      let cloneFigure = function(g) {
        if (g.kind === "ellipse") return { kind: "ellipse", cx: g.cx, cy: g.cy, rx: g.rx, ry: g.ry, info: g.info };
        return { kind: g.kind, verts: g.verts.map(function(v) { return [v[0], v[1]]; }), info: g.info };
      };
      // Point-in-figure (module space), used only by the filled-coverage guard.
      let insideFigure = function(g, px, py) {
        if (g.kind === "line") return false;
        if (g.kind === "ellipse") {
          let dx = (px - g.cx) / g.rx, dy = (py - g.cy) / g.ry;
          return dx * dx + dy * dy <= 1;
        }
        let vs = g.verts, n = vs.length, inside = false;
      ...
      let coversCanvas = function(g) {
        let corners = [[0, 0], [G, 0], [G, G], [0, G]];
        for (let i = 0; i < corners.length; i++)
          if (!insideFigure(g, corners[i][0], corners[i][1])) return false;
        return true;
      };

      // --- Lattice figure builder (module space, local frame) ---
      // Emergent tilt: a lattice edge vector v=(a,b) has perpendicular (-b,a); building a
      // polygon from such vectors keeps every vertex on the lattice while rotating the shape.
      // Sizes are integer module counts, sized to read at canvas scale (G modules ≈ canvas).
      let buildFigure = function() {
        if (shape === "Line") {
          if (regularity === "orthogonal") {
            let len = rint(4, G);
            return R.random_bool(0.5)
              ? { kind: "line", verts: [[0, 0], [len, 0]], info: { dir: "H" } }
              : { kind: "line", verts: [[0, 0], [0, len]], info: { dir: "V" } };
          }
          // diagonal: any tilted node-to-node run
          let a = rint(3, G), b = rint(3, G);
          if (R.random_bool(0.5)) b = -b;
          return { kind: "line", verts: [[0, 0], [a, b]], info: { run: a + "," + b } };
        }

        if (shape === "Circle") {
          let rx = rint(2, Math.floor(G / 2));
          if (regularity === "circle") return { kind: "ellipse", cx: 0, cy: 0, rx: rx, ry: rx, info: {} };
          let ry = rint(2, Math.floor(G / 2));
          if (ry === rx) ry = Math.max(2, rx - 1);
          return { kind: "ellipse", cx: 0, cy: 0, rx: rx, ry: ry, info: {} };
        }

        if (shape === "Square") {
          if (regularity === "rectangle") {
            let w = rint(3, G), h = rint(3, G);
            if (h === w) h = Math.max(3, w - 1);
            return { kind: "poly4", verts: [[0, 0], [w, 0], [w, h], [0, h]], info: {} };
          }
          if (regularity === "square") {
            let s = rint(3, G);
            return { kind: "poly4", verts: [[0, 0], [s, 0], [s, s], [0, s]], info: {} };
          }
          if (regularity === "tilted-square") {
            // v=(a,b) is one edge; perp(v)=(-b,a) is the next — an exact lattice square.
            let a = rint(2, 5), b = rint(1, 4);
            return { kind: "poly4", verts: [[0, 0], [a, b], [a - b, a + b], [-b, a]], info: { v: a + "," + b } };
          }
          if (regularity === "parallelogram") {
            let w = rint(3, 6), h = rint(3, 6);
            let shear = rint(1, 3) * (R.random_bool(0.5) ? 1 : -1);
            return { kind: "poly4", verts: [[0, 0], [w, 0], [w + shear, h], [shear, h]], info: { shear: shear } };
          }
          if (regularity === "trapezoid") {
            let wb = rint(4, G), wt = rint(2, wb - 1), h = rint(3, 6);
            let off = rint(0, wb - wt);
            return { kind: "poly4", verts: [[0, 0], [wb, 0], [off + wt, h], [off, h]], info: { top: wt } };
          }
          // irregular-quad: one node per side of a w×h box → convex by construction.
          let w = rint(4, G), h = rint(4, G);
          return {
            kind: "poly4",
            verts: [[rint(1, w - 1), 0], [w, rint(1, h - 1)], [rint(1, w - 1), h], [0, rint(1, h - 1)]],
            info: {}
          };
        }
        ...
        if (regularity === "isoceles") {
          // Even-length base keeps the apex on a lattice node (base midpoint is integer).
          let half = rint(2, Math.floor(G / 2)), h = rint(3, G);
          return R.random_bool(0.5)
            ? { kind: "poly3", verts: [[0, 0], [2 * half, 0], [half, h]], info: { apex: "down" } }
            : { kind: "poly3", verts: [[0, h], [2 * half, h], [half, 0]], info: { apex: "up" } };
        }
        // irregular: nodes on three sides of a box → non-degenerate, convex.
        let w = rint(4, G), h = rint(4, G);
        return { kind: "poly3", verts: [[rint(1, w - 1), 0], [w, rint(1, h - 1)], [0, h]], info: {} };
      };
      ...
      // --- Placement: integer-translate so the centroid lands in-canvas ---
      // Vertices may spill past [0, G] into the surrounding band (the cropped-field look);
      // anchoring the centroid inside [0, G] keeps the shape substantially on-canvas. ~30%
      // of the time the centroid is biased to the canvas center for isolated/centered looks.
      let placeFigure = function(g) {
        let c = centroidOf(g);
        let tx, ty;
        if (R.random_bool(0.3)) { tx = G / 2; ty = G / 2; }
        else { tx = rint(1, G - 1); ty = rint(1, G - 1); }
        translateGeom(g, Math.round(tx - c[0]), Math.round(ty - c[1]));
        // A filled figure covering all four canvas corners reads as a solid canvas — nudge
        // diagonally by one module until a corner escapes.
        if (!outline) {
          let guard = 0;
          while (coversCanvas(g) && guard < G) { translateGeom(g, 1, 1); guard++; }
        }
        return g;
      };
      ...
      // --- Convex-polygon inset (pixel space) ---
      // p5 strokes are centered on the path, so an outlined shape's ink extends swVal/2
      // beyond its lattice vertices. Offsetting every edge inward by swVal/2 (along its
      // inward normal, then re-intersecting adjacent offset edges) lands the stroke's OUTER
      // edge exactly on the original lattice position. Shapes are convex by construction and
      // swVal/2 is tiny relative to a module, so the offset lines always still intersect.
      ...
      // --- Render (module coordinates → pixels; no per-shape rotation) ---
      // Orientation comes from emergent lattice tilt plus the global canvas rotation in
      // draw(); there is no local push/rotate/pop. Each shape in a pair gets its own call,
      // color, and forceFill (see the anomaly doc above); the single-shape path always
      // passes c1 with forceFill off. When outlined, the path is inset by swVal/2 so the
      // stroke's outer edge (not its centerline) sits on the lattice position.
      let renderGeom = function(g, swVal, colorVal, forceFill) {
        let inset = outline ? swVal / 2 : 0;
        if (outline) {
          stroke(colorVal);
          strokeWeight(swVal);
          // Acute polygon corners still need to render as points rather than bevels.
        ...
        if (g.kind === "line") {
          let x1 = g.verts[0][0] * modPx, y1 = g.verts[0][1] * modPx;
          let x2 = g.verts[1][0] * modPx, y2 = g.verts[1][1] * modPx;
          // Pull the endpoints in by swVal/2 so the (round/projecting) cap ink ends on the node.
          ...
          let rx = Math.max(0, g.rx * modPx - inset);
          let ry = Math.max(0, g.ry * modPx - inset);
          ellipse(g.cx * modPx, g.cy * modPx, rx * 2, ry * 2);
        } else {
          let pts = g.verts.map(function(v) { return [v[0] * modPx, v[1] * modPx]; });
          if (inset > 0) pts = insetConvex(pts, inset);
          if (g.kind === "poly3") {
            triangle(pts[0][0], pts[0][1], pts[1][0], pts[1][1], pts[2][0], pts[2][1]);
          } else if (g.kind === "poly4") {
      ...
      // =======================================================================
      if (!secondShape) {
        let geom = placeFigure(buildFigure());
        let { swName, sw } = rollStroke();

        printShapeHeader(swName);
        print("Placement: lattice");
        let bb = bboxOf(geom);
        print("  Size (modules):", (bb.maxx - bb.minx) + "x" + (bb.maxy - bb.miny));
        let c = centroidOf(geom);
        print("  Centroid (modules): (" + (Math.round(c[0] * 10) / 10) + ", " + (Math.round(c[1] * 10) / 10) + ")");
        if (Object.keys(geom.info).length > 0) print("  Subtype info:", geom.info);

        renderGeom(geom, sw, c1);
        return;
      }

      // =======================================================================
      // Pair path — two lattice figures joined by module-cell adjacency
      // =======================================================================
      let pairMode = resolveChoice(config.pairMode, ["identical", "varied"]);
      // Line pairs are always floating: two segments meeting end-to-end just read as one
      // bent line rather than two distinct shapes.
      ...
      let geomA = buildFigure();
      let geomB = pairMode === "identical" ? cloneFigure(geomA) : buildFigure();

      // Contact: face (cardinal) or corner (diagonal) adjacency of the module bboxes.
      // Only polygons have flat faces; ellipse/line fall back to corner adjacency.
      let kindHasEdges = geomA.kind === "poly3" || geomA.kind === "poly4";
      let contactStyle = kindHasEdges ? resolveChoice(config.contactStyle, ["corner", "edge"]) : "corner";

      let bbA = bboxOf(geomA), bbB = bboxOf(geomB);
      let midXA = (bbA.minx + bbA.maxx) / 2, midYA = (bbA.miny + bbA.maxy) / 2;
      let midXB = (bbB.minx + bbB.maxx) / 2, midYB = (bbB.miny + bbB.maxy) / 2;
      let gap = pairRelation === "touching" ? 0 : rint(1, 2);
      let dx = 0, dy = 0, dir = "";
      if (contactStyle === "edge") {
        dir = R.random_choice(["right", "left", "down", "up"]);
        if (dir === "right")      { dx = (bbA.maxx - bbB.minx) + gap; dy = Math.round(midYA - midYB); }
        else if (dir === "left")  { dx = (bbA.minx - bbB.maxx) - gap; dy = Math.round(midYA - midYB); }
        else if (dir === "down")  { dy = (bbA.maxy - bbB.miny) + gap; dx = Math.round(midXA - midXB); }
        else                      { dy = (bbA.miny - bbB.maxy) - gap; dx = Math.round(midXA - midXB); }
      } else {
        let sx = R.random_bool(0.5) ? 1 : -1;
        let sy = R.random_bool(0.5) ? 1 : -1;
        dx = sx > 0 ? (bbA.maxx - bbB.minx) + gap : (bbA.minx - bbB.maxx) - gap;
        dy = sy > 0 ? (bbA.maxy - bbB.miny) + gap : (bbA.miny - bbB.maxy) - gap;
        dir = (sx > 0 ? "+x" : "-x") + (sy > 0 ? "+y" : "-y");
      }
      translateGeom(geomB, dx, dy);

      // Position the pair so the combined centroid lands in-canvas (same rule as single).
      let cA = centroidOf(geomA), cB = centroidOf(geomB);
      let ccx = (cA[0] + cB[0]) / 2, ccy = (cA[1] + cB[1]) / 2;
      let tx, ty;
      if (R.random_bool(0.3)) { tx = G / 2; ty = G / 2; }
      else { tx = rint(1, G - 1); ty = rint(1, G - 1); }
      let ptx = Math.round(tx - ccx), pty = Math.round(ty - ccy);
      translateGeom(geomA, ptx, pty);
      translateGeom(geomB, ptx, pty);

      // Filled-coverage guard: if either shape would solid-fill the canvas, nudge the whole
      // pair diagonally until it doesn't. Moving both together (rather than per shape)
      // preserves their non-overlapping relative placement — the shapes must never overlap.
      if (!outline) {
        let guard = 0;
        while ((coversCanvas(geomA) || coversCanvas(geomB)) && guard < 2 * G) {
          translateGeom(geomA, 1, 1);
          translateGeom(geomB, 1, 1);
          guard++;
        }
      }
      ...
      // --- Console output ---
      printShapeHeader(swName);
      print("Placement: lattice (pair)");
      print("  Pair Mode:", pairMode, "| Relation:", pairRelation, pairRelation === "floating" ? "| Gap: " + gap : "");
      print("  Contact:", contactStyle, "| Dir:", dir);
      print("  Anomaly:", anomaly, anomaly === "emphasis" ? "on Shape B (" + (outline ? "filled" : "blended") + ")" : "");
      let bbAf = bboxOf(geomA), bbBf = bboxOf(geomB);
      print("  Shape A — Size (modules):", (bbAf.maxx - bbAf.minx) + "x" + (bbAf.maxy - bbAf.miny));
      print("  Shape B — Size (modules):", (bbBf.maxx - bbBf.minx) + "x" + (bbBf.maxy - bbBf.miny));
      ...
      renderGeom(geomA, sw, c1);
      renderGeom(geomB, sw, colorB, forceFillB);
    }
  }
