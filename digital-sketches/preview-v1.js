...
function setup() {
  ...
  if (speedel) {
    ...
    speedel.addEventListener('input', function() {
      plotips = baseips * parseFloat(speedel.value);
      if (valel) {
        valel.textContent = speedel.value + 'x';
      }
    });
...
function buildPlotQueue(corner) {
  let batches = buildBatches(corner);
  let queue = [];
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    for (let j = 0; j < batch.cells.length; j++) {
      ...
      for (let k = 0; k < ls.length; k++) {
        ...
        svg += '      <line x1="' + x1.toFixed(6) + '" y1="' + y1.toFixed(6) +
               '" x2="' + x2.toFixed(6) + '" y2="' + y2.toFixed(6) + '"/>\n';
      }
    }
    svg += '    </g>\n';
  }
